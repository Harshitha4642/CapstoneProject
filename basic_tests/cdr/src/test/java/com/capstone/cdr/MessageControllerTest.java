...
public class MessageControllerTest {
    ...
    @Test
    @WithMockUser
    public void testGetMsgTypes() throws Exception {
        
        MessageType messageType = new MessageType();
        messageType.setId(1);
        messageType.setType("Type1");

        List<MessageType> messageTypes = Collections.singletonList(messageType);

        Mockito.when(messageService.getMsgTypes()).thenReturn(messageTypes);

        
        mockMvc.perform(MockMvcRequestBuilders.get("/api/message/msgTypes"))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().json("[{'id': 1, 'type': 'Type1'}]"));
    }
