...
export class CallFormComponent implements OnInit{
  ...
  constructor(private callService: CallService, private homeService: HomeService, private http: HttpClient){}
  ngOnInit(): void {
   this.callType = this.callService.getType();
   console.log("LEt us see how things work");
    console.log();
    if(localStorage.getItem("processedLocationsCSV") === 'false'){
      console.log("nothing in local storage, so process");
      this.loadLocationsFromCSV("/assets/in.csv");
    }
      else
      {
        console.log("something in database, lets retrieve");
  ...
  //   console.log(type);
  //   console.log(this.form.reciever);
  //   console.log(this.form.subscriber);
  //   if(this.form.subscriber === 0 || this.form.reciever === 0)
  //   {
  //     this.status.status = "Enter valid user ID";
  //   } 
  //   if(!this.form.reason || !this.form.callType )
  //   {
  //     this.status.status = "Enter non empty values";
  //   }
  //   if (new Date(this.form.date) > new Date()) {
